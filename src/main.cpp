...
int main(int argc, const char **argv) {
	std::shared_ptr<GLTF::Asset> asset(new GLTF::Asset());
	...
	if (parser->parse(argc, argv)) {
		...
		std::shared_ptr<COLLADA2GLTF::ExtrasHandler> extrasHandler(new COLLADA2GLTF::ExtrasHandler(loader));
		std::shared_ptr<COLLADA2GLTF::Writer> writer(new COLLADA2GLTF::Writer(asset, &options, extrasHandler));
		loader->registerExtraDataCallbackHandler((COLLADASaxFWL::IExtraDataCallbackHandler*)extrasHandler.get());
		COLLADAFW::Root root(loader.get(), writer.get());
