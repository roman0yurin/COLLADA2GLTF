...
TEST(GLTFAccessorTest, CreateFromData) {
  ...
  std::shared_ptr<GLTF::Accessor> accessor(new GLTF::Accessor(GLTF::Accessor::Type::VEC3,
    GLTF::Constants::WebGL::FLOAT,
    (unsigned char*)points, 4,
    GLTF::Constants::WebGL::ARRAY_BUFFER
  ));
  float* min = accessor->min.get();
  ASSERT_TRUE(min != NULL);
  EXPECT_EQ(min[0], 1.0);
  EXPECT_EQ(min[1], 2.0);
  EXPECT_EQ(min[2], 3.0);

  float* max = accessor->max.get();
...
TEST(GLTFAccessorTest, CreateOnBuffer) {
  ...
  std::shared_ptr<GLTF::Accessor> accessorFromData(new GLTF::Accessor(GLTF::Accessor::Type::VEC3,
    GLTF::Constants::WebGL::FLOAT,
    (unsigned char*)pointsBuffer,
    4,
    GLTF::Constants::WebGL::ARRAY_BUFFER
  ));
  std::shared_ptr<GLTF::BufferView> bufferView = accessorFromData->bufferView;
  float points[6] = {13.0, 14.0, 15.0, 16.0, 17.0, 18.0};
  std::shared_ptr<GLTF::Accessor> accessor(
          new GLTF::Accessor(GLTF::Accessor::Type::VEC3,
            GLTF::Constants::WebGL::FLOAT,
