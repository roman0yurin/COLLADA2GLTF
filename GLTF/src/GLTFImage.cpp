...
GLTF::Image::Image(std::string uri, std::string cacheKey, std::vector<uint8_t> *data, size_t byteLength, std::string fileExtension) :
		uri(uri), byteLength(byteLength), cacheKey(cacheKey)
{
	this->data.reset(data);
	std::string dataSubstring((char*)data->data(), 8);
...
GLTF::Image::Image(std::string uri, std::vector<uint8_t> *data, size_t byteLength, std::string fileExtension) : Image(uri, "", data, byteLength, fileExtension) {}



std::shared_ptr<GLTF::Image> GLTF::Image::load(path imagePath, std::map<std::string, std::shared_ptr<GLTF::Image>> &imageCache) {
	std::string fileString = imagePath.string();
	std::map<std::string, std::shared_ptr<GLTF::Image>>::iterator imageCacheIt = imageCache.find(fileString);
	if (imageCacheIt != imageCache.end()) {
	...
	if (file == NULL) {
		...
		image.reset(new GLTF::Image(imagePath.filename().string(), fileString));
	}
	else {
		...
		std::vector<uint8_t> *buffer = new std::vector<uint8_t>(size, 0);
		int bytesRead = fread(buffer->data(), sizeof(unsigned char), size, file);
		fclose(file);
		image.reset(new GLTF::Image(imagePath.filename().string(), fileString, buffer, bytesRead, fileExtension));
	}
	imageCache[fileString] = image;
	return image;
}
...
std::pair<int, int> GLTF::Image::getDimensions() {
	...
	if (mimeType == "image/png") {
		uint32_t* readUInt32 = (uint32_t*)(data->data() + 16);
		...
		while (offset < byteLength) {
			uint16_t* readUint16 = (uint16_t*)(data->data() + offset);
			i = endianSwap16(readUint16[0]);
			next = (data->data() + offset)[i + 1];

			// 0xFFC0 is baseline(SOF)
			// 0xFFC2 is progressive(SOF2)
			if (next == 0xC0 || next == 0xC2) {
				readUint16 = (uint16_t*)(data->data() + offset + i + 5);
...
void GLTF::Image::writeJSON(void* writer, GLTF::Options* options) {
	...
	if (options->embeddedTextures && data != NULL) {
		if (!options->binary) {
			jsonWriter->Key("uri");
			std::string embeddedUri = "data:" + mimeType + ";base64," + Base64::encode(data.get(), byteLength);
